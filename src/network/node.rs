...
impl<S: Storage, N: Network> Node<S, N> {
    ...
    pub async fn store(&self, key: &[u8], value: Vec<u8>) -> Result<()> {
        ...
        let result = match tokio::time::timeout(Duration::from_secs(5),
                                               self.protocol.store_value(key_id.clone(), value.clone())).await {
            Ok(r) => {
        ...
        for node in &nodes {
            if node.id != self.node_id {
                ...
                match tokio::time::timeout(Duration::from_secs(2),
                                          self.protocol.store(node, key_id.clone(), value.clone())).await {
    ...
    pub async fn get(&self, key: &[u8]) -> Result<Vec<u8>> {
        ...
        let find_result = match tokio::time::timeout(Duration::from_secs(5),
                                                    self.protocol.find_value(&key_id)).await {
            Ok(result) => {
        ...
        for node in &nodes {
            if node.id != self.node_id {
                ...
                match tokio::time::timeout(Duration::from_secs(2),
                                          self.protocol.find_value_rpc(node, key_id.clone())).await {
                    Ok(Ok((Some(value), _))) => {
        ...
        match self.protocol.ping(&bootstrap_node).await {
            Ok(response) => {
                ...
                // Manually update our routing table with the bootstrap node from response
                {
                    ...
                    println!("Then added bootstrap node from response to routing table. Total nodes: {}", node_count);
                ...
                match self.protocol.store(&bootstrap_node, mykey_id.clone(), myvalue_bytes.clone()).await {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if buckets_count > 0 {
                    ...
                    for _ in 0..8 { // Refresh 8 random IDs
