...
async fn test_ping_request_response() -> Result<()> {
    ...
    // リクエストを送信
    sender_udp.send(receiver_addr, KademliaMessage::Request(request)).await?;
...
async fn test_find_node_request_response() -> Result<()> {
    ...
    // レスポンスを検証
    match response {
        ResponseMessage::NodesFound { request_id, sender: _, nodes } => {
            assert_eq!(request_id, message_id, "リクエストIDが一致しません");
...
async fn test_find_value_request_response() -> Result<()> {
    ...
    // レスポンスを検証
    match response {
        ResponseMessage::ValueFound { request_id, sender: _, value, nodes } => {
            assert_eq!(request_id, message_id, "リクエストIDが一致しません");
...
async fn test_store_request_response() -> Result<()> {
    ...
    // レスポンスを検証
    match response {
        ResponseMessage::StoreResult { request_id, sender: _, success } => {
            assert_eq!(request_id, message_id, "リクエストIDが一致しません");
...
async fn test_timeout_handling() -> Result<()> {
    ...
    // リクエストを送信
    sender_udp.send(non_existent_addr, KademliaMessage::Request(request)).await?;
...
async fn test_custom_request_handler() -> Result<()> {
    ...
    // リクエストを送信
    sender_udp.send(receiver_addr, KademliaMessage::Request(request.clone())).await?;
